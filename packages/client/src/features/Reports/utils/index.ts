...
const reportsMiddleware = async ({
  entityType,
  filter,
  filterFilter,
  from,
  to,
}: T.ReportsMiddlewareArgs) => {
...
const setGraphicsState = ({
  ...
  group.chart.entities = data.map(
    ({ period, entities }: { period: string; entities: T.EntityData[] }) => {
      const point = {
  ...
  group.statistics = metadata.entities.map(
    ({ id, entityType, subscribe, leave }: T.EntityData) => {
      const row = {
        id,
        entityType,
        subscribe,
      ...
      row.subscribersAPS = data[indexFirst].entities.filter(
        (el: T.EntityData) => el.id === id,
      ).subscribe;
      row.subscribersAPE = data[indexLast].entities.filter(
        (el: T.EntityData) => el.id === id,
      ).subscribe;
      ...
      row.percentages =
        100 *
        Math.abs(
          (subscribersAPS - subscribersAPE) /
            ((subscribersAPS + subscribersAPE) / 2),
        );
