...
const Graphics: FC<Props> = ({
  ...
  const { ids } = useSelector(
    () => store.getState()[entityType === 0 ? 'networks' : 'events'],
  );
  ...
  const handlePeriodEffect = () => {
    ...
    if (
      [T.Period.THIS_YEAR, T.Period.LAST_MONTH, T.Period.LAST_WEEK].includes(
        filterFilter,
      )
    ) {
      dispatch(
  ...
  const handleRangeChange = ({
    ...
    let { from, to } = store.getState().reports[entityType][T.PERIOD][
