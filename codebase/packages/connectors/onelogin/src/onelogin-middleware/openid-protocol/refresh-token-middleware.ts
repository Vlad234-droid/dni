...
const getKey = (client: JwksClient): GetPublicKeyOrSecret => (
  header,
  callback,
): void => {
...
export const getRefreshTokenMiddleware = <TClient extends Client>({
  ...
  return asyncHandler(async (req, res, next) => {
    logger(
      LoggerEvent.info(
        'verification',
        `Refresh token middleware: validating auth cookie (${authDataCookie.name})`,
        { req, res },
      ),
    );
    ...
    if (!authData) {
      throw new OneloginError(
        'verification',
        `Cookie not set: ${printCookieInfo("authdata", authDataCookie)}`,
        401,
      );
    }
    ...
    if (!requireIdToken) {
      logger(
        LoggerEvent.debug('verification', "IdToken not required", { req, res }),
      );
      return next();
    }
    ...
    if (idToken == null || encRefreshToken == null) {
      throw new OneloginError(
        'verification',
        "Missing idToken or encRefreshToken",
        401,
      );
    }
    ...
    const handleTokenValid = () => {
      logger(
        LoggerEvent.debug('verification', "IdToken is valid", { req, res }),
      );
    ...
    const handleTokenExpired = async () => {
      logger(
        LoggerEvent.debug('verification', "IdToken expired, refreshing", { req, res }),
      );
      ...
      const refreshTokenSet = async () => {
        ...
        logger(
          LoggerEvent.debug('verification', "IdToken token refreshed successfully", { req, res }),
        );
    ...
    const verifyResult = await verifyJwt<OpenIdUserInfo>(
      idToken,
      getKey(jwksClientInstance),
    );
