...
import {
  openIdAuthDataMiddleware,
  AuthData,
  setOpenIdAuthData,
  getOpenIdAuthData,
} from "../../auth-data-extractor";
import {
  persistentTracingMiddleware,
...
export const getOpenidMiddleware = async (
  configuration: OpenidConfig,
): Promise<OpenIdRouter> => {
  const {
    clientId,
    clientSecret,
  ...
  const clearCookies = (res: express.Response) => {
    res
      .clearCookie(authDataCookie.name)
      .clearCookie(SESSION_COOKIE_NAME)
      .clearCookie(`${SESSION_COOKIE_NAME}.sig`);
  };
  ...
  router.get(AUTHENTICATION_PATH, (req, res, next) => {
    logger(
      LoggerEvent.info(
        "login",
        "Authentication path handler was called - clearing cookies and authenticating",
        { req, res },
      ),
    );

    clearCookies(res);
    OpenId.authenticationHandler(req, res, next);
  });

  const callbackPathHandler = asyncHandler(async (req, res, next) => {
    logger(
      LoggerEvent.info("login", "Redirect URI handler was called", {
        req,
        res,
      }),
    );
    ...
    if (req.query && req.query.error != null) {
      ...
      throw new OneloginError(
        "login",
        errorMsg as string,
        req.query.error === "access_denied" ? 403 : undefined,
      );
    }
  ...
  const createCookie = asyncHandler(async (req, res) => {
    if (res.writableEnded) {
      logger(LoggerEvent.warn("login", "Response is alredy completed. Create cookies aborted", { req, res }));
      return;
    }

    logger(LoggerEvent.info("login", "Creating cookies...", { req, res }));

    if (!req.cookies || !req.signedCookies)
      throw Error("cookie-parser with correct key is required");

    const authData = requireIdToken ? getOpenIdAuthData(res) : {};
    ...
    const validate = () => {
      const validationStatus = validateCookies({
        ...req.cookies,
        ...req.signedCookies,
        [authDataCookie.name]: authDataFromCookie,
      });
      ...
      if (!validationStatus.valid) {
        logger(
          LoggerEvent.info(
            "login",
            `Cookie validation reponse: ${validationStatus.message}`,
            { req, res },
          ),
        );
      }
    ...
    logger(
      LoggerEvent.info(
        "login",
        `Following cookies were created:
          - ${printCookieInfo(
            `authdata${requireIdToken ? "" : " (empty)"}`,
            authDataCookie,
          )}
  ...
  router.get(LOGOUT_PATH, (req, res) => {
    ...
    const afterLogoutRedirectUrl =
      redirectAfterLogoutUrl || `${applicationPath}${AUTHENTICATION_PATH}`;

    logger(
      LoggerEvent.info(
        "logout",
        `User will be redirected to the ${afterLogoutRedirectUrl}`,
        { req, res },
      ),
    );
...
const extractMetadataFromIssuer = ({
  metadata,
}: Issuer<Client>): IssuerMetadataLight => {
