...
export const identityApiDef = defineAPI((endpoint) => ({
  getToken: endpoint.post('/identity/v4/issue-token/token').response<ApiOutput>().body<ApiBody>().build(),
}));

export const identityApiConnector = (ctx: IdentityApiContext) => {
  const baseUrl = resolveBaseUrl(TESCO_API_URLS, ctx);

  const apiConsumer = createApiConsumer(identityApiDef, fetchClient(baseUrl, {}, ctx));
