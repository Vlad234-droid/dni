...
const getList = createAsyncThunk<T.Notification[]>(A.GET_LIST_ACTION, async () => {
  const list = S.serializeNotification(await API.notifications.fetchAll<T.NotificationResponse[]>());
...
const getListGroupByNetwork = createAsyncThunk<T.NetworkNotification[]>(A.GET_LIST_NETWORKS_ACTION, async () => {
  const list = S.serializeNetworkNotification(
    await API.notifications.fetchAllGroupByNetwork<T.NetworkNotificationResponse[]>(),
  );
