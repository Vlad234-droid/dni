import React, { FC, HTMLProps, ChangeEvent, useRef, useState, useEffect } from 'react';
...
type Props = HTMLProps<HTMLInputElement> & FieldProps & { onChange: (file: File) => void };
    ...
    <Wrapper {...{ error }}>
      <div>
        <label htmlFor={name}>
          ...
          <input ref={inputEl} type='file' id={name} name={name} onChange={handleAttachFile} hidden />
        </label>
        <br />
        {file && file.name}
      </div>
